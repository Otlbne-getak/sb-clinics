...
//data collection
	...
	$appointment_id = (int) test_inputs($_POST['appointment_id']);
		...
		$confirmed_time = $app_data['confirm_time'];
		...
		if($confirmed_time == NULL){
			//not confirmed
			$conf_data = ", `confirm_time` = '".$date_time."' ";
		}
		
		
		$q = "UPDATE `clinics_appointments` SET `status` = '5', `check_in_time` = '".$date_time."' ".$conf_data." WHERE ((`appointment_id` = '".$appointment_id."') AND (`dr_id` = ".$dr_id.") AND (`clinic_id` = ".$_SESSION['clinic_id']."))";

		
			if(mysqli_query($KONN, $q)){
				
				die(lang('Appointment_Checked_In'));
