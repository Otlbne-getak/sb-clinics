...
//data collection
	...
	$appointment_id = (int) test_inputs($_POST['appointment_id']);
		...
		$q = "UPDATE `clinics_appointments` SET `status` = '3', `cancel_time` = '".$date_time."' WHERE ((`appointment_id` = '".$appointment_id."') AND (`dr_id` = ".$dr_id.") AND (`clinic_id` = ".$_SESSION['clinic_id']."))";

		
			if(mysqli_query($KONN, $q)){
				
				die(lang('Appointment_Canceled'));
