...
//data collection
	...
	$app_duration = (int) test_inputs($_POST['duration']);
		...
		//check if reserved
		$q_02 = "SELECT COUNT(appointment_id) FROM `clinics_appointments` WHERE ((time_date = '".$app_date."') AND (`dr_id` = ".$dr_id.") AND (`clinic_id` = ".$_SESSION['clinic_id']."))";
		...
		//die("0|==".$endTime);
		if($patient_id == 0 ){
			die('0|ERROR no : Please Select Patient');
		}
		
		//place appointment
			//insert patient data
			$q = "INSERT INTO `clinics_appointments` (
				`procedure_id`, 
				`time_date`, 
				`end_time_date`, 
				`app_duration`, 
			...
			if(mysqli_query($KONN, $q)){
				...
				die('1|'.lang('New_Appointment_Added'));
